-- ============================================================
-- init-db.sql
-- Merged: init-db + tax_documents.sql + client_profiles.sql
-- ============================================================
...
CREATE TABLE IF NOT EXISTS public.tax_documents (
    ...
    embedding_model text NULL,
    embedding_pipeline_version text NULL,
    -- Kolom tambahan dari tax_documents.sql
    avoidance_signals jsonb NULL,
    thresholds jsonb NULL,
    exception_refs _text NULL,
    conflict_refs jsonb NULL,
    applicable_entity_types _text NULL,
    gaar_exposure_level text NULL,
    effective_date_end date NULL,
    sunset_clause bool DEFAULT false NULL,
...
CREATE INDEX IF NOT EXISTS idx_tax_docs_avoidance_signals ON public.tax_documents USING gin (avoidance_signals);
...
CREATE INDEX IF NOT EXISTS idx_tax_docs_entity_types ON public.tax_documents USING gin (applicable_entity_types);
CREATE INDEX IF NOT EXISTS idx_tax_docs_gaar_exposure ON public.tax_documents USING btree (gaar_exposure_level) WHERE (gaar_exposure_level IS NOT NULL);
...
CREATE INDEX IF NOT EXISTS idx_tax_docs_thresholds ON public.tax_documents USING gin (thresholds);
...
CREATE TABLE IF NOT EXISTS public.client_profiles (
    ...
    klu_code text NULL,
    klu_description text NULL,
    omzet_setahun int8 NULL,
    is_pkp bool DEFAULT false NULL,
    npwp text NULL,
    domisili_provinsi text NULL,
    tahun_berdiri int4 NULL,
    laba_sebelum_pajak int8 NULL,
    total_aset int8 NULL,
    total_hutang int8 NULL,
    total_ekuitas int8 NULL,
    der_ratio numeric(5, 2) NULL,
    komponen_biaya jsonb NULL,
    penghasilan_pasif jsonb NULL,
    nilai_aset_tetap int8 NULL,
    penyertaan_saham jsonb NULL,
    kredit_pajak jsonb NULL,
    ada_pemegang_asing bool DEFAULT false NULL,
    persentase_asing numeric(5, 2) NULL,
    negara_pemegang_asing _text NULL,
    ada_afiliasi bool DEFAULT false NULL,
    daftar_afiliasi jsonb NULL,
    spt_history jsonb NULL,
    transaksi_afiliasi jsonb NULL,
    rencana_bisnis text NULL,
    sengketa_aktif bool DEFAULT false NULL,
    detail_sengketa jsonb NULL,
    CONSTRAINT client_profiles_client_code_key UNIQUE (client_code),
    CONSTRAINT client_profiles_pkey PRIMARY KEY (id)
);

CREATE INDEX IF NOT EXISTS idx_client_profiles_entity_type ON public.client_profiles USING btree (entity_type);
CREATE INDEX IF NOT EXISTS idx_client_profiles_omzet ON public.client_profiles USING btree (omzet_setahun);

-- Trigger client_profiles
